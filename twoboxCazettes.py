...
class twoboxCazettesMDP:
    def __init__(self, discount, nq, nr, na, nl, parameters, lick_state=False):
        ...
        self.loc0_mask = np.ones((self.na, self.nq*self.nq*self.nr*self.nlwh*self.nlpbh))
        self.loc0_mask[pb, :2*self.nq*self.nr*self.nq*self.nlpbh] = 0
    ...
    def _states_wLickStates(self, r_, l_, a_):
        temp = np.reshape(np.array(range(self.nq)), [1, self.nq])
        return np.squeeze(l_ * self.nq * self.nq * self.nr * self.nlpbh + a_ * self.nq * self.nq * self.nr + 
    ...
    def setupMDP(self):
        ...
        if isinstance(self.parameters , ArrayBox):
            ...
            Groom = self.parameters._value[2]   # location 0 reward
            travelCost = self.parameters._value[3]
            pushButtonCost = self.parameters._value[4]
            startPushButtonCost = self.parameters._value[5]
            stopPushButtonCost = 0.0#self.parameters._value[6]
            portRestReward = self.parameters._value[6]
        else:
        ...
        Tb_unpack = {bloc:{oloc:{} for oloc in belief_locations} for bloc in belief_locations}
        ...
        ac_history_labels = np.array([str(pb) + '_' + str(loc) for loc in range(2)])
        ...
        if self.lick_state:
            self.ThA_labels = kronn_str(loc_mesh, ac_history_mesh, bel1_mesh, rew_mesh, bel2_mesh)
            self.R_labels = tensorsumm_str(loc_labels_with_history[:, None], ac_history_labels[:, None],
                                           bel1_labels[None, :], 
        ...
        if self.lick_state:
            #if no button is pushed then there is no state transition from
            #the previous button history state to the next button history state
            #this means if you hadnt pushed a button before, then you still havent
            #and vice versa.
            Tpb0 = np.array([[1, 1],
                             [0, 0]])
            #on the other hand if the push button action is taken then if the agent
            #had not pushed the button before, then it will transition to the button
            #now having been pushed and if the button had been pushed before it STILL
            #will now have been pushed before.
            Tpb1 = np.array([[0, 0], 
                             [1, 1]])
            Tla0 = kronn(np.identity(self.nlwh), Tpb0)
            Tlg0 = kronn(Tl, Tpb0)
            Tlpb = kronn(Tpb0, np.identity(self.nq), Tr, np.identity(self.nq))
            # Tlpb = kronn(Tpb0, Tb_unpack[1][1][a0], Tr, Tb_unpack[2][2][a0])
        else:
            Tla0 = np.identity(self.nlwh)
            Tlg0 = Tl
            Tlpb = kronn(np.identity(self.nq), Tr0, np.identity(self.nq))

        #make per location belief transition matrices
        Tb = {bloc:{} for bloc in belief_locations}
        for action in actions:
            for bloc in belief_locations:#belief location is the location of the agent
                ...
                Tb_block_temp = np.block([
                                    [Tb_temp_n0, Tb_temp_n0],
                                    [Tb_temp_n1, Tb_temp_n1]
                                    ])
                oloc = [l for l in belief_locations if l != bloc][0]#should only be one other location
                if bloc == 1:
                    Tb[bloc][action] = kronn(Tpb1, Tb_block_temp, Tb_unpack[bloc][oloc][action])
                else:
                    Tb[bloc][action] = kronn(Tpb1, Tb_unpack[bloc][oloc][action], Tb_block_temp)
                    
                # #normalize columns
                # # Tb_block_temp = Tb_block_temp / np.sum(Tb_block_temp, axis=0)
                # #get the oloc
                
                # if self.lick_state:
                #     Tb[bloc][action] = kronn(Tpb1, Tb_block_temp, Tb_unpack[bloc][oloc][action])
                # else:
                #     Tb[bloc][action] = kronn(Tb_block_temp, Tb_unpack[bloc][oloc][action])
            
        # ACTION: do nothing
        self.ThA[a0, :, :] = kronn(Tla0, np.identity(self.nq), Tr, np.identity(self.nq))
        ...
        self.ThA[g0, :, :] = kronn(Tlg0, np.identity(self.nq), Tr, np.identity(self.nq))
        # self.ThA[g0, :, :] = kronn(Tlg0, Tb_unpack[1][1][g0], Tr, Tb_unpack[2][2][g0])

        # ACTION: push button.
        # Th = block_diag(kronn(np.identity(self.nq), Tr, np.identity(self.nq)), Tb[1][pb], Tb[2][pb])
        ...
        Th = block_diag(Tlpb, Tlpb, Tb[1][pb], Tb[2][pb])
        ...
        if self.lick_state:
            ...
            Reward_h = tensorsumm(loc_rewards, np.zeros((1, self.nlpbh)), np.zeros((1, self.nq)), np.array([[0, r_val]]),
                              np.zeros((1, self.nq)))
            ...
            Reward_h_pb = tensorsumm(loc_rewards_pb, np.zeros((1, self.nlpbh)), np.zeros((1, self.nq)), np.array([[0, r_val]]),
                              np.zeros((1, self.nq)))
            ...
            [R1, R2, R3] = np.meshgrid(Reward_a.T, Reward_h_pb, Reward_h_pb, indexing='ij')
            ...
            lick_state0 = np.concatenate((r0_l2_a0, r0_l3_a0, r1_l2_a0, r1_l3_a0))
            #check that the indices are unique
            assert len(lick_state0) == len(np.unique(lick_state0))
            lick_state0 = np.unique(lick_state0)#sort the indices
            ...
            lick_state1 = np.concatenate((r0_l2_a1, r0_l3_a1, r1_l2_a1, r1_l3_a1))
            #check that the indices are unique
            assert len(lick_state1) == len(np.unique(lick_state1))
            lick_state1 = np.unique(lick_state1)#sort the indices
            row_inds, col_inds = np.ix_(lick_state0, lick_state1)
            Reward[pb, row_inds, col_inds] += -startPushButtonCost
            row_inds, col_inds = np.ix_(lick_state1, lick_state0)
            Reward[a0, row_inds, col_inds] += -stopPushButtonCost
            Reward[g0, row_inds, col_inds] += -stopPushButtonCost

            #add a reward for taking a rest at the port in between push button actions
            Reward[a0, :, 2*self.nq * self.nq * self.nr * self.nlpbh:] += portRestReward
            ...
            Reward[pb, :, :2*self.nq * self.nr * self.nq] += -pushButtonCost*10
        ...
        den_col = {bloc:{} for bloc in belief_locations}
        for action in actions:
            for bloc in belief_locations:#belief location is the location of the agent
                obs_dict = {}
                for i in range(NumCol):
                    ...
                    if bloc == 1:
                        if self.lick_state:
                            obs_dict[(i, j)] = kronn(Tpb1, den_block_temp, den_loc[oloc][action][j])
                        else:
                            obs_dict[(i, j)] = kronn(den_block_temp, den_loc[oloc][action][j])
                    else:
                        if self.lick_state:
                            obs_dict[(i, j)] = kronn(Tpb1, den_loc[oloc][action][j], den_block_temp)
                        else:
                            obs_dict[(i, j)] = kronn(den_loc[oloc][action][j], den_block_temp)
        ...
        for i in range(NumCol):
            j = i
            self.Trans_hybrid_obs12[i, j, a0, :, :] = kronn(Tla0, np.identity(self.nq), Tr, np.identity(self.nq)).T
            self.Trans_hybrid_obs12[i, j, g0, :, :] = kronn(Tlg0, np.identity(self.nq), Tr, np.identity(self.nq)).T
            #build out blocks of Trans_hybrid_obs12, there will be blocks for each location in Tl, so self.nlwh
            #first for a0
            self.Trans_hybrid_obs12[i, j, pb, :, :] = block_diag(Tlpb, Tlpb, den_col[1][pb][(i,j)], den_col[2][pb][(i,j)]).T
        # for k1 in range(self.nlwh):#rows of Tl (next state)
        #     for k2 in range(self.nlwh):#columns of Tl (current state)
        #         if k2 <= 1:#if current location is in the intermediate location..
        #             bloc = 1#arbitrary choice of belief location
        #         else:#if the current location is in one of foraging locations
        #             bloc = (k2 % 2) + 1 #column ind 2 is location 1 coming from 0, so belief location is 1
                    
        #         a0_block_k = kronn(Tpb0, self.den[bloc][1][a0][0], Tr, self.den[bloc][2][a0][0]) * np.eye(self.nlwh)[k1, k2]
        #         g0_block_k = kronn(Tpb0, self.den[bloc][1][g0][0], Tr, self.den[bloc][2][g0][0]) * Tl[k1, k2]
        #         block_next_state_inds = np.arange(k1 * self.nq * self.nq * self.nr * self.nlpbh, (k1 + 1) * self.nq * self.nq * self.nr * self.nlpbh)#next state indices
        #         block_curr_stat_inds = np.arange(k2 * self.nq * self.nq * self.nr * self.nlpbh, (k2 + 1) * self.nq * self.nq * self.nr * self.nlpbh)#current state indices
        #         #to make indices for Trans_hybrid_obs12 remember we are flipping the rows and columns
        #         #s.t. the NOW current state is the row and the next state is the column
        #         block_inds = np.ix_(block_curr_stat_inds, block_next_state_inds)
        #         self.Trans_hybrid_obs12[0, 0, a0, block_inds[0], block_inds[1]] = a0_block_k.T
        #         self.Trans_hybrid_obs12[0, 0, g0, block_inds[0], block_inds[1]] = g0_block_k.T
        #         self.Trans_hybrid_obs12[1, 1, a0, block_inds[0], block_inds[1]] = a0_block_k.T
        #         self.Trans_hybrid_obs12[1, 1, g0, block_inds[0], block_inds[1]] = g0_block_k.T
...
class twoboxCazettesMDPdata(twoboxCazettesMDP):
    def __init__(self, discount, nq, nr, na, nl, parameters, parametersExp,
                 sampleTime, sampleNum, lick_state=False):
        ...
        for n in range(self.sampleNum):
            ...
            for t in range(self.sampleTime):
                if t == 0:
                    ...
                    if self.lick_state:
                        pb_ind = 0
                        self.hybrid[n, t] = self.location_ind[n, t] * (self.nq * self.nr * self.nq * 2) + \
                    ...
                    if self.action[n, t - 1] != pb:
                        ...
                        #     self.belief1Dist[n, t] = self.den[self.location[n, t - 1]][1][self.action[n, t - 1]][0, :, self.belief1[n, t - 1]]
                            
                        #     self.belief2Dist[n, t] = self.den[self.location[n, t - 1]][2][self.action[n, t - 1]][0, :, self.belief2[n, t - 1]]
                        # else:
                        #     self.belief1Dist[n, t] = self.den[1][1][self.action[n, t - 1]][self.reward[n, t], :, self.belief1[n, t - 1]]
                        #     self.belief2Dist[n, t] = self.den[2][2][self.action[n, t - 1]][self.reward[n, t], :, self.belief2[n, t - 1]]
                    ...
                    if self.action[n, t - 1] == pb:  # press button
                        ...
                        if self.location[n, t] == 1:  # consider location 1 case
                            if self.trueState1[n, t - 1] == 0:#if the box is inactive
                                ...
                                self.trueState1[n, t] = np.random.binomial(1, 1 - psw_e)
                                self.color1[n, t] = self.trueState1[n, t]
                                self.trueState2[n, t] = abs(1 - self.trueState1[n, t])
                                self.color2[n, t] = self.trueState2[n, t]
                        ...
                        if self.location[n, t] == 2:  # consider location 2 case
                            if self.trueState2[n, t - 1] == 0:
                                ...
                                #there is now a chance psw_e that the box will switch off
                                self.trueState2[n, t] = np.random.binomial(1, 1 - psw_e)
                                self.color2[n, t] = self.trueState2[n, t]
                                self.trueState1[n, t] = abs(1 - self.trueState2[n, t])
                                self.color1[n, t] = self.trueState1[n, t]
...
class twoboxCazettesIndependentDependentMDP:
    ...
    def setupMDP(self):
        ...
        # ACTION: do nothing
        self.ThA[a0, :, :] = kronn(np.identity(self.nlwh), np.identity(self.nq), Tr, np.identity(self.nq))
